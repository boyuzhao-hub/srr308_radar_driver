...
namespace FHAC
{
      ...
      const rclcpp_lifecycle::State &)
  {
    ...
    can_filter radar_cluster_quality_filter{
        ID_Cluster_2_Quality,
        id_mask,
    };
  ...
  void radar_conti_ars408::handle_cluster_list(std::shared_ptr<const polymath::socketcan::CanFrame> frame)
  {
    ...
    if (Get_MsgID0_From_MsgID(frame->get_id()) == ID_Cluster_1_General)
    {
      // cluster ID
  ...
  void radar_conti_ars408::publish_cluster_map(int sensor_id)
  {
    ...
    {
      visualization_msgs::msg::Marker mcluster;

      mcluster.header.stamp = rclcpp_lifecycle::LifecycleNode::now();
      mcluster.header.frame_id = radar_link_names_[sensor_id];
      mcluster.ns = "cluster";
      mcluster.id = itr->first;
      mcluster.type = visualization_msgs::msg::Marker::SPHERE;
      mcluster.action = visualization_msgs::msg::Marker::ADD;
      mcluster.pose.position.x = itr->second.cluster_general.cluster_distlong.data;
      mcluster.pose.position.y = itr->second.cluster_general.cluster_distlat.data;
      mcluster.pose.position.z = 0.0;

      double yaw = 0.0; // Cluster does not have orientation information

      tf2::Quaternion myQuaternion;
      myQuaternion.setRPY(0, 0, yaw);
      mcluster.pose.orientation.w = myQuaternion.getW();
      mcluster.pose.orientation.x = myQuaternion.getX();
      mcluster.pose.orientation.y = myQuaternion.getY();
      mcluster.pose.orientation.z = myQuaternion.getZ();

      // SRR308 Change: To draw different color of sphere for different radar
      if (sensor_id == 0) {
        mcluster.color.r = 1.0; mcluster.color.g = 0.0; mcluster.color.b = 0.0; // left front radar
        } else if (sensor_id == 1) {
        mcluster.color.r = 0.0; mcluster.color.g = 0.0; mcluster.color.b = 1.0; // right front radar
        }
      mcluster.color.a = 1.0;

      mcluster.lifetime = rclcpp::Duration::from_seconds(0.5);

      // Size of the cluster can be represented by the RCS value, we can scale it for better visualization
      double rcs_value = itr->second.cluster_general.cluster_rcs.data;
      double scale_factor = std::max(0.3, std::min(2.0, rcs_value * 0.1)); // Scale factor for better visualization
      mcluster.scale.x = scale_factor;
      mcluster.scale.y = scale_factor;
